...
import { TREASURY_ADDR, FEE_TREASURY_BPS } from '../config.js'
...
export async function applyTxFeeAndReward(state, proposerAddr, txCount) {
  ...
  // Split fees to treasury vs proposer (basis points out of 10000)
  const toTreasury = (feeTotal * BigInt(FEE_TREASURY_BPS)) / 10000n;
  const toProposer = feeTotal - toTreasury;

  // credit treasury
  const tAddr = TREASURY_ADDR.toLowerCase();
  const treasury = st.accounts[tAddr] || { balance: '0', nonce: 0 };
  treasury.balance = (BigInt(treasury.balance) + toTreasury).toString();
  st.accounts[tAddr] = treasury;

  // credit proposer: block reward + leftover fee (if any)
  const prop = st.accounts[proposerAddr] || { balance: '0', nonce: 0 };
  prop.balance = (BigInt(prop.balance) + blockReward + toProposer).toString();

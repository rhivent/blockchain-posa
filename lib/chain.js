...
async function applyGenesisAllocations(state) {
  const allocPath = path.join(process.cwd(), 'genesis.alloc.json')
  if (!fs.existsSync(allocPath)) return state
  try {
    const raw = JSON.parse(fs.readFileSync(allocPath, 'utf8'))
    const alloc = raw.alloc || {}
    const st = JSON.parse(JSON.stringify(state))
    for (const [addr, amt] of Object.entries(alloc)) {
      const a = addr.toLowerCase()
      const v = BigInt(amt)
      const acc = st.accounts[a] || { balance:'0', nonce:0 }
      acc.balance = (BigInt(acc.balance) + v).toString()
      st.accounts[a] = acc
    }
    return st
  } catch (e) {
    console.error('[genesis.alloc] invalid file:', e?.message || e)
    return state
  }
}
 ...
 export async function initGenesis() {
   ...
   // apply optional genesis allocations
   state = await applyGenesisAllocations(state)
